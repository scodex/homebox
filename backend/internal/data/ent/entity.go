...
// was not loaded in eager-loading.
func (e EntityEdges) TagOrErr() ([]*Tag, error) {
	if e.loadedTypes[3] {
		return e.Tag, nil
	}
...
func (*Entity) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case entity.ForeignKeys[1]: // entity_type_entities
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
...
// QueryTag queries the "tag" edge of the Entity entity.
func (_m *Entity) QueryTag() *TagQuery {
	return NewEntityClient(_m.config).QueryTag(_m)
}
